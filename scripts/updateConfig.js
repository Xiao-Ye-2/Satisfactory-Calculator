...
async function updateTranslations(items, recipes, machines) {
  ...
  try {
    await execPromise(`python ${path.join(__dirname, 'translate.py')} ${enPath} ${zhPath}`);
    ...
    const finalZhTranslations = { ...generatedZhTranslations, ...zhDefault };
...
async function main() {
  try {
    ...
    console.log('Configuration and translation files have been successfully updated');
  } catch (error) {
